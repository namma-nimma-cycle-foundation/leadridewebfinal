...
various action buttons for the meeting. The `elements` key contains the various
action buttons.
...
The `autoTune` property returns a boolean value If true, force the participant's
video to be in compatibility mode.
...
`maxVideoCount` - number of participants to show in the grid. `videoFit` -
string value suggesting how to align the video content.
