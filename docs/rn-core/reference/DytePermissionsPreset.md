...
The `requestProduce` returns boolean value. If `true`, participant can send
request to participants about producing audio, video or screenshare.
...
The `requestKickParticipant` returns boolean value. If `true`, participant can
request to remove another participant from the meeting.
...
possible values are `SKIP_ON_ACCEPT`, `ON_ACCEPT`,
...
boolean values, `canCreate`, `canVote`, `canView`
...
There are 3 permissions `allow` - if video is allowed `quality` - quality of the
video `frameRate` - frame rate of the video
...
The `produceScreenshare` shows permissions for enabling screen share for the
participant. There are 3 permissions `allow` - if sharing screen is allowed
`quality` - quality of the video `frameRate` - frame rate of the video
...
The `produceAudio` shows permissions for enabling audio for the participant. If
true, participant can enable audio.
...
The `reactions` returns boolean value. If `true`, reactions are enabled for the
participant
...
The `canChangeTheme` returns boolean value. If `true`, the participant can
change the meeting theme.
...
The `canPresent` returns boolean value. If `true`, the participant can become a
presentor.
